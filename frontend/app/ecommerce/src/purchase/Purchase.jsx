...
import QuantitySelector from '../cart/components/QuantitySelector'; // Asegúrate de que la ruta sea correcta
...
export default function Purchase() {
  const { cart, deleteProduct, purchaseCart, updateQuantityProduct } = useCart();
  ...
  useEffect(() => {
    if (cart && cart.payload && cart.payload.products && cart.payload.products.length > 0) {
      const calculatedSubtotal = cart.payload.products.reduce((acc, item) => acc + item.product.price * item.quantity, 0);
    ...
    <section className="p-4 w-full max-w-[1200px] mx-auto mt-[100px]">
      ...
      {cart.payload.products.length > 0 ? (
            ...
            <table className="w-full border border-[#61005D] rounded-lg overflow-hidden">
              ...
              <tbody>
                {cart.payload.products.map((item) => (
                  <tr key={item.product._id} className="border-b hover:bg-gray-100">
                    <td className="py-2 px-4">
                      <img className="w-24 h-24 object-cover rounded" src={item.product.thumbnails[0]} alt={item.product.title} />
                    </td>
                    <td className="py-2 px-4 text-center">{item.product.title}</td>
                    <td className="py-2 px-4 text-center">${formatNumber(item.product.price)}</td>
                    <td className="py-2 px-4 text-center">
                      <QuantitySelector
                        quantity={item.quantity}
                        onIncrease={() => handleIncrease(item.product._id, item.quantity)}
                        onDecrease={() => handleDecrease(item.product._id, item.quantity)}
                      />
                    </td>
        ...
        <section className="flex justify-center items-center flex-col h-full">
          <p className="text-center my-4 text-lg font-semibold">Tu carrito se encuentra vacío.</p>
        </section>
      )}
    </section>
  );
